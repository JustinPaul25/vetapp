...
const submitVerification = () => {
    verificationForm.post('/email/verify-code', {
        preserveScroll: true,
        onSuccess: () => {
...
const resendCode = () => {
    resendForm.post('/email/resend-verification', {
        preserveScroll: true,
    });
};
...
<template>
    <AuthLayout
        title="Verify email"
        description="Please verify your email address by entering the code we sent to you."
    >
        ...
        <div class="flex flex-col items-center gap-6">
            ...
            <div
                v-if="status === 'verification-link-sent' || status === 'verification-code-sent'"
                class="w-full p-4 bg-green-50 border border-green-200 rounded-lg text-center"
            >
                <div class="flex items-center justify-center gap-2 text-green-700">
                    <CheckCircle2 class="h-5 w-5" />
                    <span class="text-sm font-medium">A new verification code has been sent to the email address you provided during registration.</span>
                </div>
            </div>
